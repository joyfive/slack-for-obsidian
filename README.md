...
좋아요. 아래는 `README.md`에 추가하기 적합한 **확장 계획 항목 내용 정리**입니다.
기존 스타일에 맞춰 `## 🚀 Milestones & Sequencing` 또는 별도 `## 🔮 Extension Plan` 섹션에 넣을 수 있어요.

---

## 🔮 Extension Plan: Preview & Editor 기능 고도화 (예정)

### 1. OAuth 도입

> `env`를 활용한 개인용 앱을 넘어서기 위한 슬랙 OAuth 로그인을 통한 유저 확장 방향

#### 1.1.✳️ 기능 확장 개요

- 현재 개인용 봇토큰과 로그인 토큰을 `env`로 관리하며 단 한명의 사용자(기쁨)만을 위한 미니앱으로 구현되어있지만,  
  추후 다른 사용자들도 로그인 시 앱 설치 -> 설치된 앱을 원하는 채널에 초대하여 자유롭게 메시지를 수집할 수 있는 오픈 서비스로 확장하고자 함
...
| 기능                                 | 설명                                                                |
| ------------------------------------ | ------------------------------------------------------------------- |
| 슬랙 OAuth 로그인 도입               | 최초 로그인 시 워크스페이스에 앱 설치, 이후 봇 토큰을 반환받는 형식 |
| 채널/메시지 조회 등 슬랙 API 호출 시 | 로그인 시점에 반환받은 사용자별 봇토큰을 활용                       |

#### 1.3.📈 기대 효과

> 유연한 사용자 확장을 통해, 실제 사용자의 반응을 살펴볼 수 있는 미니 앱으로의 확장

### 2. 마크다운 편집기

#### 2.1.✳️ 기능 확장 개요

- 현재 `.md` 파일은 다운로드만 지원하지만,
  추후에는 추출된 마크다운을 웹에서 직접 **미리보기 및 편집**할 수 있는 화면으로 확장할 계획
...
| 기능                             | 설명                                                                   |
| -------------------------------- | ---------------------------------------------------------------------- |
| `/preview` 페이지 전환           | 메시지 추출 후 전용 페이지(`/preview`)로 이동하여 `.md` 내용 확인 가능 |
| **미리보기 & 편집** swap         | 미리보기(pre) ↔ 수정(textarea) 전환 가능                               |
| 퀵 복사 버튼                     | 편집 후 즉시 클립보드 복사 가능 (`📋 복사`)                            |
| 다운로드 버튼                    | 수정된 내용 그대로 `.md`로 저장 (`⬇️ 다운로드`)                        |
| 옵시디언/다른 툴과의 연결성 향상 | iOS 단축어 / 외부 툴 연계 전송 가능성 확보                             |

---

#### 2.3.📈 기대 효과

- `.md` 파일 다운로드 없이도 **빠른 메시지 정제 & 전환** 가능
- 블로그 글쓰기, Obsidian 기록 이전 단계에서 **에디팅 루프** 가능
- 추후 "기록 저장소"나 "템플릿 적용" 등으로 확장 가능
